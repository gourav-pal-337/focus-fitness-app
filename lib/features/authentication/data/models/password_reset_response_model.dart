import 'register_response_model.dart';

/// Response model for requesting password reset
class PasswordResetRequestResponseModel {
  PasswordResetRequestResponseModel({
    required this.success,
    required this.message,
    required this.expiresAt,
  });

  final bool success;
  final String message;
  final String expiresAt;

  factory PasswordResetRequestResponseModel.fromJson(
    Map<String, dynamic> json,
  ) {
    return PasswordResetRequestResponseModel(
      success: json['success'] as bool? ?? false,
      message: json['message'] as String? ?? '',
      expiresAt: json['expiresAt'] as String? ?? '',
    );
  }
}

/// Response model for confirming password reset
class PasswordResetConfirmResponseModel {
  PasswordResetConfirmResponseModel({
    required this.success,
    required this.message,
    required this.user,
    this.tokens,
    this.token,
    this.accessToken,
    this.refreshToken,
  });

  final bool success;
  final String message;
  final UserModel user;
  final TokensModel? tokens;
  final String? token;
  final String? accessToken;
  final String? refreshToken;

  factory PasswordResetConfirmResponseModel.fromJson(
    Map<String, dynamic> json,
  ) {
    return PasswordResetConfirmResponseModel(
      success: json['success'] as bool? ?? false,
      message: json['message'] as String? ?? '',
      user: json['user'] != null
          ? UserModel.fromJson(json['user'] as Map<String, dynamic>)
          : throw Exception('User data is required'),
      tokens: json['tokens'] != null
          ? TokensModel.fromJson(json['tokens'] as Map<String, dynamic>)
          : null,
      token: json['token'] as String?,
      accessToken: json['accessToken'] as String?,
      refreshToken: json['refreshToken'] as String?,
    );
  }
}

